...
<DemoVideo
  ...
  kind="vhs"
  caption="A tool firing inside a live attempt: the agent invokes Attempt_Get, the call + arguments render in the invocation trace (call.start → call.complete), and the structured result feeds back into the assistant's next turn."
/>
