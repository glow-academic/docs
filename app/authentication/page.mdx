...
sequenceDiagram
    ...
    M->>K: verify signature
    M->>M: resolve_identity to profile + session
    M-->>C: handler runs as the resolved caller
```
...
The curl examples throughout these docs read two environment variables —
your instance URL and your bearer token — so you can **export them once
and every example is copy-paste-ready**:
...
# …or paste a token directly: export GLOW_TOKEN=eyJhbG...
```

`GLOW_INSTANCE_URL` is the same variable the CLI reads; `GLOW_TOKEN` is the
...
{/* DEMO_VIDEO: authentication-service-accounts | vhs */}
