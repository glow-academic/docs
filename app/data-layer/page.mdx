...
sequenceDiagram
    ...
    C->>MV: 1st search cold, response cached
    C->>R: 2nd search route-cache hit
    Note over C,R: mutation elsewhere fires X-Invalidate-Tags
    C->>MV: 3rd search clean miss, re-warms
```
...
```bash
curl -X POST $GLOW_INSTANCE_URL/persona/refresh \
