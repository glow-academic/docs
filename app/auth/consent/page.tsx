...
function ConsentFlow({ onComplete }: { onComplete: () => void }) {
    ...
    fetch("/api/auth/oauth2/consent", {
      ...
      .then((res) => {
        ...
        return res.json().then((data) => {
          ...
          // After redirect, show done state and try to close
          setTimeout(() => {
            onComplete()
            window.close()
          }, 1500)
...
export default function ConsentPage() {
  ...
  return (
    <div style={{ textAlign: "center", maxWidth: "20rem", width: "100%", padding: "0 1rem" }}>
      <div style={{ background: "white", borderRadius: "1rem", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", border: "1px solid #e5e7eb", padding: "2.5rem 2rem" }}>
        {!done ? (
          ...
          <>
            ...
            <h1 style={{ fontSize: "1.125rem", fontWeight: 600, color: "#111827", marginBottom: "0.25rem" }}>Authorized</h1>
            <p style={{ fontSize: "0.875rem", color: "#6b7280", marginBottom: "1.5rem" }}>You can close this window.</p>
          </>
        )}
      </div>
      ...
      <Suspense><ConsentFlow onComplete={() => setDone(true)} /></Suspense>
    </div>
  )
}
