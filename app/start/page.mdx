...
Equivalent raw HTTP (no `/v5` prefix, singular paths). Export
`$GLOW_INSTANCE_URL` + `$GLOW_TOKEN` once and the examples throughout
these docs are copy-paste-ready — see [Authentication](/authentication#raw-http):
