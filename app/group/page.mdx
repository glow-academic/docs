...
curl -X POST $GLOW_INSTANCE_URL/system/group \
...
<DemoVideo
  ...
  kind="vhs"
  caption="A run group two ways via POST /system/groups: the lean table (id, run count, cost, time span) for quick scanning, then the full JSON row behind a line — group id, run count, exact first/last run timestamps, and session — for audit linking."
/>
